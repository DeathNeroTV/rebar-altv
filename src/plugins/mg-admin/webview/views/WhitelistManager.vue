<script lang="ts" setup>
	...
	const deleteRequest = async (index: number) => {
		events.emitServer(AdminEvents.toServer.whitelist.delete, entries.value[index]._id);
	...
	<div class="p-6 text-gray-100 flex flex-col gap-6 select-none w-full h-full">
		...
		<div v-if="entries.length > 0" class="grid grid-cols-4 gap-6">
			<div
				...
				:key="entry.code"
				class="bg-neutral-900 border border-neutral-800 rounded-2xl p-5 flex-col justify-between shadow-md hover:shadow-xl transition-all hover:-translate-y-1"
			>
				<!-- Header -->
				...
				<div class="flex justify-center gap-3 mt-2">
					<button @click.stop="approveRequest(index)" class="bg-green-600 hover:bg-green-700 p-2 rounded-full text-sm font-medium transition cursor-pointer">
						<font-awesome-icon :icon="['fas', 'check']" />
					</button>
					<button @click.stop="rejectRequest(index)" class="bg-orange-600 hover:bg-orange-700 p-2 rounded-full text-sm font-medium transition cursor-pointer">
						<font-awesome-icon :icon="['fas', 'xmark']" />
					</button>
					<button @click.stop="deleteRequest(index)" class="bg-red-600 hover:bg-green-700 p-2 rounded-full text-sm font-medium transition cursor-pointer">
