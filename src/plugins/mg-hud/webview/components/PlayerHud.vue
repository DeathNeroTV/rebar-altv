		...
		<div class="w-full h-full flex flex-col gap-1">
			...
			<div class="relative flex w-full h-3 gap-2">
				...
				<div class="absolute top-0 left-0 flex h-full w-full gap-0.5">
					<div v-for="i in 12" :key="'armor-fg-linear-' + i" class="flex-1 h-full" :class="{ 'bg-gray-100': i <= Math.ceil(data.armour / (100 / 12)) }"></div>
				</div>
			</div>
...
<script lang="ts" setup>
	...
	onMounted(async () => {
		data.value = await events.emitServerRpc(HudEvents.toServer.loadPlayer);
