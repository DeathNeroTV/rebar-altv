<script lang="ts" setup>
	import { Modifiers } from '@Plugins/mg-inventory/shared/interfaces';
	import { ref } from 'vue';

	type ClickType = 'none' | 'left' | 'middle' | 'right';

	type EventCallbacks = {
		onLeftClick: [id: string, modifiers: Modifiers];
	...
	function handleClick(ev: MouseEvent) {
		...
		clickType.value = clickTypes[ev.button] || 'none';
		...
		if (Date.now() - lastClickTime.value <= 150) {
	...
	function stop(ev: MouseEvent) {
		...
		if (totalDragTime <= 120) {
			emits('onLeftClick', id.value as string, { shift: ev.shiftKey, ctrl: ev.ctrlKey, alt: ev.altKey });
			return;
		}
