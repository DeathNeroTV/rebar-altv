...
const selectedCategory = ref<
  "all" | "medical" | "eatable" | "weapons" | "drugs" | "resources"
>("all");
...
const displayedItems = computed(() => {
  ...
  // Wenn kein Filter aktiv → zeige Inventar exakt so wie es ist
  if (!hasSearch && !hasCategory) {
    return slots;
  }

  // Andernfalls: Nur Items zeigen, die passen (ohne Leerslots)
  ...
  // Optional – falls du willst, dass Gridgröße gleich bleibt:
...
// Klick-Events
function handleLeftClick(uid: string, modifiers: { shift: boolean; ctrl: boolean }) {
...
<template>
  <div
    class="flex flex-col gap-2 p-5 bg-neutral-950/90 rounded-3xl w-full h-full overflow-hidden"
  >
    <!-- Ghost Item -->
    <div
      v-if="draggingItem"
      class="pointer-events-none z-[9999] w-24 h-24 fixed"
      :style="{ top: ghostStyle.top, left: ghostStyle.left }"
    >
    ...
    <div class="flex gap-5 items-center">
      <div class="w-1/2 flex flex-row gap-2 p-2 rounded-full bg-neutral-800 items-center">
        ...
        <input
          ...
          v-model="searchQuery"
          placeholder="Suchen..."
          class="flex-1 bg-neutral-800 text-gray-100 focus:outline-none placeholder-neutral-600"
        />
      </div>
      <div
        class="w-1/2 flex flex-row gap-5 text-gray-100 text-3xl items-center justify-end"
      >
        <font-awesome-icon
          ...
          :class="[
            selectedCategory === cat.value ? 'text-[#008736]' : 'hover:text-[#008736]',
          ]"
        />
      </div>
    </div>
    ...
    <div class="w-full h-full overflow-y-auto">
      <div class="w-full h-full grid grid-cols-5 gap-3 p-2">
        <Draggable
        ...
        >
          ...
          >
            ...
            <span
              ...
              class="absolute bottom-1 right-1 text-xs text-gray-100 bg-neutral-900 px-2 py-1 rounded"
            >
              {{ item.quantity }}
            </span>
          </div>
        ...
        <template v-else v-for="item in displayedItems">
          <div
            v-if="item"
            @mouseenter="emits('hoverOver', item)"
            @mouseleave="emits('hoverOver', null)"
    ...
    <div v-if="hasBar" class="flex gap-2 justify-center">
      <div
        v-for="n in 4"
        :key="n"
        class="w-full h-24 bg-neutral-800 rounded-lg border border-neutral-700 flex items-center justify-center text-white font-semibold"
      >
        {{ n }}
      </div>
    </div>
