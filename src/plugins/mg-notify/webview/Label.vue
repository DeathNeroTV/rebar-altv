...
<script setup lang="ts">
	...
	const transitionLabelName = computed(() => {
		if (MGNotifications.textlabelPosition.includes('left')) {
			return 'label-slide-left';
		} else if (MGNotifications.textlabelPosition.includes('right')) {
	...
	const labelPositionClass = computed(() => {
		...
		return `${positions[MGNotifications.textlabelPosition]} z-30 space-x-4`;
	});
	...
	const init = () => {
		if (!showLabel.value && labels.value === undefined) {
			events.emitServer(NotifyEvents.toServer.sendLabelDataToServer);
		}

		events.on(NotifyEvents.toWebview.createLabel, (label: Label) => {
		...
		events.on(NotifyEvents.toWebview.removeLabel, () => {
			removeTextLabel();
		});
	};
