...
export function sendNotification(player: alt.Player, notification: VueNotification) {
    ...
    if (notificationToSend.oggFile && MGNotifications.sounds) {
        Rebar.player.useAudio(player).playSound(`/sounds/${notificationToSend.oggFile}.ogg`);
    }

    view.emit(NotifyEvents.toWebview.createNotification, notificationToSend);
}
...
export function sendDebugNotification(player: alt.Player, message: string) {
    if (!MGNotifications.debug) return;
...
export function createTextlabel(player: alt.Player, label: Label) {
    ...
    if (labelToSend.oggFile && MGNotifications.labelSound) {
    ...
    view.emit(NotifyEvents.toWebview.createLabel, labelToSend);
}
